...
<div class="globe-markers" v-show="globeCtrl.isVisible">
  <div class="globe-marker__wrapper" 
  ...
  >
    <div id="globe-marker-{{location.userData.id}}" class="globe-marker icon-basic-location-pin-2"
      :class="{
    ...
    >
            <span class="globe-marker__count" v-if="location.userData.locations.length > 1 && !globeCtrl.highlightedPlants.length">
                {{location.userData.locations.length}}
            </span>
            <span class="globe-marker__count" v-if="globeCtrl.highlightedPlants.length && location.userData.matchCount">
                {{location.userData.matchCount}}
            </span>
            <div class="globe-marker__labels globe-marker__labels--desktop" 
              v-if="!globeCtrl.isMobileView()" 
              v-show="globeCtrl.activeMarker == location.userData.id"
            >
                <div class="globe-marker__labels__wrapper">
                    <ul>
                        <li class="globe-marker__label globe-marker__label--desktop" ng-repeat="location in location.userData.locations track by $index" :class="{'is-no-match': !globeCtrl.isSubplantHighlighted(location) }">
                            
                            <a class="globe-marker__label__icon icon-da-03-location-marker-list"></a>
                            <div class="globe-marker__label__cta">
                                <div class="button" @click="globeCtrl.moveToIvLocation($event,location.sitemodelId,'panorama')">
                                    <span class="button__icon icon-bp-02-view-panorama "></span>
                                    <span class="button__text">Panorama</span>
                                </div>
                                <div class="button" @click="globeCtrl.moveToIvLocation($event,location.sitemodelId,'map')">
                            ...
                            <div class="globe-marker__label__info">
                                <p class="globe-marker__label__subplant" v-if="location.state.highlighted" ng-bind-html="location.state.matchingTitle">
                                    <p class="globe-marker__label__subplant" v-if="!location.state.highlighted"> {{location.label}}
                                    </p>
                            </div>
        ...
        <div class="globe-marker__labels globe-marker__labels--mobile" v-if="globeCtrl.isMobileView()" v-show="globeCtrl.activeMarker == location.userData.id">
            <div class="globe-marker__labels__wrapper">
                <ul>
                    <li class="globe-marker__label globe-marker__label--mobile" ng-repeat="location in location.userData.locations track by $index" :class="{ 'is-selected': globeCtrl.selectedPlant === location.id, 'is-no-match': !globeCtrl.isSubplantHighlighted(location) }" @click="globeCtrl.onPlantSelected($event,location.id)">
                        <a class="globe-marker__label__icon icon-da-03-location-marker"></a>

                        <div class="globe-marker__label__cta" v-show="globeCtrl.selectedPlant === location.id">
                            <div class="button" @click="globeCtrl.moveToIvLocation($event,location.sitemodelId,'panorama')">
                                <span class="button__icon icon-bp-02-view-panorama"></span>
                                <span class="button__text">Panorama</span>
                            </div>
                        ...
                        <div class="globe-marker__label__info">
                            <breadcrumb class="globe-marker__label__breadcrumb" show-sub-building="false" building-id="location.id" :class="{'is-inline': !crumbCtrl.showSubBuilding }">
                            </breadcrumb>
                            <p class="globe-marker__label__subplant" v-if="location.state.highlighted" ng-bind-html="location.state.matchingTitle"></p>
                            <p class="globe-marker__label__subplant" v-if="!location.state.highlighted"> {{location.label}}
                            </p>
